...
// Heatmap Data matching the Heatmap page
...
export default function LiveNetworkWidget() {
  ...
  const totalNodes = nodes.length > 0 ? nodes.length : 8;
  const onlineCount = nodes.filter((n) => n.status === 'active').length || totalNodes;

  return (
    <div
      className={styles.card}
      onClick={() => router.push('/heatmap')}
      title="Click to open Heatmap"
      role="button"
      tabIndex={0}
    >
      <div className={styles.header}>
        <div>
          <h3 className={styles.title}>Live Network</h3>
          <span className={styles.subtitle}>Geographic patient distribution</span>
        </div>
        <span className={styles.openLink}>
          Heatmap <ArrowUpRight size={13} />
        </span>
      </div>
      ...
      <div className={styles.mapArea}>
        ...
        >
          {/* Indian States with Heatmap Colors */}
